...
int main() {
    ...
    // 💡 救命關鍵：將預設的 5000ms 逾時縮短為 100ms，保證 Server 絕對不卡頓！
    sensorClientConfig->timeout = 100; 
    ...
    while(running) {
        ...
        if (!sensorConnected) {
            if (time(NULL) - last_try >= 3) {
                last_try = time(NULL);
                // 💡 使用 127.0.0.1 避開 DNS 查詢地雷
                if(UA_Client_connect(sensorClient, "opc.tcp://127.0.0.1:4842") == UA_STATUSCODE_GOOD) {
