...
static void onDistanceDataChange(UA_Server *server, const UA_NodeId *sessionId, void *sessionContext, const UA_NodeId *nodeId, void *nodeContext, const UA_NumericRange *range, const UA_DataValue *value) {
    ...
    // 2. 檢查資料是否帶有 Publisher 的來源時間戳
    if (value->hasSourceTimestamp) {
        // 計算時間差 (UA_DateTime 單位為 100 奈秒)
        // 1 毫秒 = 10,000 個單位
        UA_Int64 timeDiff = now - value->sourceTimestamp;
        
        // 設定防禦閾值：如果這個封包是 500 毫秒前發出的，判定為延遲或重播攻擊，拒絕處理！
        if (timeDiff > 5000000) { 
            UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, 
                           "⚠️ [警告] 收到過期封包 (延遲 %.1f ms)，可能為重播攻擊，已丟棄！", 
                           (double)timeDiff / 10000.0);
            return; // 直接 return，不驅動馬達
        }
    }
